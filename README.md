此仓库的特定功能部分将在约两周后进行回收处理并择机在原仓库emo emb版本release后合并重组

> 这些东西可能会在意想不到的用途和要求上派上用场
...
~~仍在开发中~~ ~~是否放弃开发待定~~ ~~此仓库不被认为应该继续开发研究，因此已于2023-10-23放弃~~ 更正于2023-11-10:新特性开发方向已被基本放弃。该项目最后一次更新应用了新g2p算法与来自**HTSEngine**的基本语调/停顿辅助信息。彻底重写后的g2p算法(与1.1.1master分支相比)大幅提升数据利用率，鲁棒性与准确度(具体表现为g2p报错概率极大幅度地降低并提升bert特征信息计算流程准确性)
